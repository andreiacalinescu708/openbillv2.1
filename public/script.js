 ...
 if (Array.isArray(obj)) {
  obj.forEach(item => {
    ...
    if (typeof item === "string") {
      ...
      // ✅ IMPORTANT: trimitem ID dacă există
      value = (item.id != null) ? String(item.id) : (item.name || "");
    } else {
      return;
    }

    const b = document.createElement("button");
    b.type = "button";           // ✅ să fie sigur că nu “submit”
    b.className = "itembtn";

    // (păstrează colorarea alezelor dacă vrei)
...
if (searchInput && resultsBox) {
  ...
  renderTree(tree, (val) => {
    ...
    // ✅ 1) caută după ID
    let p = flat.find(x => String(x.id) === key);

    // ✅ 2) fallback: după nume (ca înainte)
    if (!p) p = flat.find(x => String(x.name || "").trim() === key);

    if (!p) {
      console.warn("Produs negăsit pentru:", val);
      alert("Produs negăsit în catalog (tree ≠ flat).");
      return;
    }
...
async function initOrdersPage() {
  ...
  function getFilterState() {
    return {
      ...
      q: searchInput ? String(searchInput.value || "").toLowerCase().trim() : ""
    };
  }
  ...
  function setStatusFilter(v) {
    ...
    if (tabsBox) {
      [...tabsBox.querySelectorAll(".mTab")].forEach(b => {
        b.classList.toggle("active", String(b.dataset.status || "") === val);
      });
    }
  }
  ...
  function render() {
    ...
    if (q) {
      filtered = filtered.filter(o => {
        const name = String(o?.client?.name || "").toLowerCase();
  ...
  if (selStatus) selStatus.onchange = () => {
    setStatusFilter(selStatus.value || "");
    render();
  };
